...
// --- ESCUCHADOR DEL BOTÓN "VOLVER" FÍSICO DE ANDROID ---
window.addEventListener('popstate', function(event) {
    const estado = event.state;
    if (!estado) return;

    // 1. Si el estado es una vista interna del Perfil
    if (estado.subVistaPerfil) {
        navegarPerfil(estado.subVistaPerfil, false);
    }
    // 2. Si el estado es la vista de Categorías (Grilla)
    else if (estado.subVistaStock === 'categorias') {
        mostrarInventario(false);
    }
    // 3. Si el estado es la vista de Productos dentro de una Categoría
    ...
    // 4. Si es una pestaña principal (Ventas, Estadísticas, etc)
    else if (estado.pestana) {
        // Buscamos el botón del menú inferior que corresponde a esta pestaña
        const botonCorrespondiente = document.querySelector(`[onclick*="${estado.pestana}"]`);
        cambiarPestana(estado.pestana, botonCorrespondiente, false);
    }
});