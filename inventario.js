...
// 1. DIBUJAR LA GRILLA DE CATEGORÍAS AL INICIAR
...
// --- AQUÍ INSERTAMOS EL TÍTULO ---
    ...
    titulo.style.cssText = "color: #1a1a1a; text-align: left; width: 100%; margin-bottom: 15px; font-size: 18px; font-weight: bold;";
    ...
    categoriasDefinidas.forEach(cat => {
        ...
        // --- AQUÍ ESTÁ LA MODIFICACIÓN DEL BRILLO ---
        div.style.cssText = `
        ...
        // También le agregué un pequeño 'drop-shadow' a la imagen para que destaque sobre el fondo brillante
        div.innerHTML = `
...
function aplicarFiltrosStock() {
    ...
    filtrados.forEach(prod => {
        ...
        if (prod.fechaVencimiento) {
            infoVencimiento = `<small style="display:block; color: ${ordenarPorFecha ? '#dc3545' : '#856404'}; font-weight: bold;">Vence: ${prod.fechaVencimiento}</small>`;
        }

        item.innerHTML = `
            <div style="text-align: left; width: 65%;">
                <strong style="display:block; font-size: 16px;">${prod.nombre}</strong>
                <small style="color:#666; display:block;">Cód: ${prod.id}</small>
                ${infoVencimiento}
            </div>
            <div style="text-align: right; width: 35%;">
                <div style="font-weight: bold; color:#29AB50; font-size: 16px;">$${prod.precio}</div>
                <div style="font-size: 13px; color:#555; background: #e9ecef; padding: 2px 5px; border-radius: 5px; display: inline-block; margin-top: 5px;">Stock: <strong>${prod.stock}</strong></div>
            </div>
        `;
