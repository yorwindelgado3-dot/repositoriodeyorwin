...
function aplicarFiltrosStock() {
    ...
    filtrados.forEach(prod => {
        ...
        if (prod.fechaVencimiento) {
            infoVencimiento = `<small style="font-size: 10px; "display:block; color: ${ordenarPorFecha ? '#dc3545' : '#856404'}; font-weight: bold;">Vence: ${prod.fechaVencimiento}</small>`;
        }

        item.innerHTML = `
            <div style="text-align: left; width: 65%;">
                <strong style="display:block; font-size: 12px;">${prod.nombre}</strong>
                <small style="color:#666; display:block; font-size: 10px;">Cód: ${prod.id}</small>
                ${infoVencimiento}
            </div>
            <div style="text-align: right; width: 35%;">
                <div style="font-weight: bold; color:#29AB50; font-size: 12px;">$${prod.precio}</div>
                <div style="font-size: 10px; color:#555; background: #e9ecef; padding: 2px 5px; border-radius: 5px; display: inline-block; margin-top: 5px;">Stock: <strong>${prod.stock}</strong></div>
            </div>
        `;
