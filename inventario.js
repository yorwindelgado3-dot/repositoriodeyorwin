...
function mostrarInventario(guardarEnHistorial = true) {
    ...
    if (guardarEnHistorial) {
        // Evitamos guardar duplicados si ya estamos en categorías
        if (!window.history.state || window.history.state.subVistaStock !== 'categorias') {
            history.pushState({ pestana: 'tab-stock', subVistaStock: 'categorias' }, "", "");
        }
    }
}

// 2. ABRIR UNA CATEGORÍA ESPECÍFICA
function abrirCategoria(catId, catNombre, tieneVencimiento, guardarEnHistorial = true) {
    ...
    if (guardarEnHistorial) {
        history.pushState({ pestana: 'tab-stock', subVistaStock: 'productos' }, "", "");
    }
}
