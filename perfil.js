...
function exportarInventario() {
    ...
    // Convertimos los productos a un texto JSON bonito (indentado)
    const dataStr = JSON.stringify(productos, null, 2);
    // Creamos un archivo virtual (Blob)
    const blob = new Blob([dataStr], { type: "application/json" });
    const url = URL.createObjectURL(blob);

    // Creamos un enlace invisible, le damos clic y lo destruimos
    const a = document.createElement('a');
    a.href = url;
    const fecha = new Date().toLocaleDateString('es-ES').replace(/\//g, '-');
    a.download = `Inventario ${fecha}.json`; 
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url); // Liberamos memoria
    mostrarAlertaCustom("¡Exportación exitosa! Revisa la carpeta de 'Descargas' de tu teléfono. Si deseas guardarlo en otro lugar, usa el botón de Compartir.");
}
