...
function ejecutarVentaFinal() {
    ...
    // --- NUEVO PASO 1: Arreglo para detectar qué productos se van a agotar ---
    ...
    for (let i = 0; i < carrito.length; i++) {
        ...
        // --- NUEVO PASO 2: Si vendes todo lo que queda, lo anotamos en la lista ---
        if (itemCar.cantidad === stockTotalDisponible) {
            productosAgotados.push(itemCar.nombre);
        }
    }
    ...
    // --- NUEVO PASO 3: LIMPIEZA DE INVENTARIO ---
    // Filtramos la base de datos para guardar SOLO los productos que tengan 1 o más de stock.
    // Los que quedaron en 0 se borran automáticamente.
    bd[usuarioActual].productos = bd[usuarioActual].productos.filter(p => (parseInt(p.stock) || 0) > 0);

    // FASE 3: REGISTRO HISTÓRICO DE LA VENTA
    ...
    const nuevaVenta = {
        ...
        articulos: carrito.map(item => ({
            ...
            categoriaId: item.categoriaId || "CAT-OTRO"
        })),
    ...
    // Actualizar estadísticas y la lista de inventario visualmente
    if (typeof renderizarEstadisticas === "function") renderizarEstadisticas();
    if (typeof cargarProductosDeCategoria === "function") cargarProductosDeCategoria();

    // --- NUEVO PASO 4: ALERTA DINÁMICA ---
    let mensajeFinal = "Venta procesada correctamente.";
    if (productosAgotados.length > 0) {
        // Usamos \n para hacer saltos de línea porque la alerta usa innerText
        mensajeFinal += "\n\nATENCIÓN: Se agotó por completo el stock de:\n- " + productosAgotados.join("\n- ");
    }

    mostrarAlerta("¡Éxito!", mensajeFinal);
}
