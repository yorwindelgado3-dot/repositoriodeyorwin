...
function renderizarGraficoEvolucion(ventas) {
    ...
    ventas.forEach(venta => {
        ...
        if (modoFiltro === 'dia') {
            ...
            // Usamos la fecha (YYYY-MM-DD)
            clave = venta.fecha.split('T')[0];
        }
...
function renderizarGraficoCategorias(ventas, productos) {
    ...
    productos.forEach(p => {
        ...
        // ¡EL ERROR ESTABA AQUÍ! Usamos categoriaId en lugar de categoria
        const cat = p.categoriaId && p.categoriaId.trim() !== '' ? p.categoriaId : 'Sin Categoría';
    ...
    ventas.forEach(venta => {
        venta.articulos.forEach(art => {
            ...
            // Si por algún motivo el artículo ya traía su propio categoriaId guardado
            if (!categoriaCode && art.categoriaId) {
                categoriaCode = art.categoriaId;
            }

            categoriaCode = categoriaCode || 'Sin Categoría';
    ...
    top3.forEach((prod, index) => {
        contenedor.innerHTML += `
            <div style="display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px solid #eee;">
                ...
                <span style="color: #666;">[${prod.cantidad} und]</span>
            </div>
        `;
    });
}
